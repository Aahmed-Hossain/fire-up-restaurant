...
const AddedFoodList = ({ addedFood, refetch }) => {
  const { _id, image, quantity, price, date, food_name, email,  } =
    addedFood;
  ...
  const handleDelete = (id) => {
    swal({
      ...
      if (willDelete) {
        ...
        axiosHook.delete(`/allFoods/${id}`).then((res) => {
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 md:grid-cols-3 items-center justify-between gap-2  border border-slate-200 rounded-xl mb-4 px-0 md:px-2 lg:px-2 ">
        ...
        <div className="flex items-center justify-center gap-3">
          <button
            ...
            className="btn-circle btn-sm opacity-50 hover:opacity-70 bg-black text-white hidden md:block lg:block px-3 ml-1"
          >
            X
          </button>
          <img
            className="lg:w-[15rem] md:w-[11rem] w-full rounded-xl flex items-start"
            src={image}
            alt=""
          />
        </div>
        ...
        <div className="flex items-center lg:justify-end md:justify-end justify-between gap-4 p-2 md:p-0 lg:p-0 ">
          ...
          <button
            onClick={() => handleDelete(_id)}
            className=" btn-circle btn-sm opacity-50 hover:opacity-70 bg-black text-white px-3 visible md:hidden lg:hidden"
          >
            X
          </button>
        </div>
